...
description: >-
  Use when designing or creating a new Grok Bot: ask preference questions, write
  a tight persona, CreateAgent, then verify against the live profile. Coding bots
  use pstack / poteto-mode; on fresh install run setup-pstack first.
---
# Design a Grok Bot

## When

- Designing or creating a new Grok Bot
- First real user message after this template is imported (fresh-install setup)
- User asks for a coding or non-coding bot with a clear one-job

## Do

Create the bot. Do not ship a shareable template unless the user asked for a template. `CreateAgent` takes `name` and `description` only — that description is the whole persona. There is no delete tool; only create when the job is real.
...
Do not paste the full pstack playbook into the description. If pstack is not installed, say so and keep the same tightness anyway.
...
1. Read the live profile back from `/home/box/agent-data/agents/<id>/profile.json`. Do not trust the tool ack alone.
2. Tell the user the name and the one-job line. Mention they delete from the sidebar (right-click, Delete) if they hate it.
3. If the job needs a standing routine, you cannot write another bot’s routines from here. Immediately SendToAgent that new bot with an explicit instruction to call `update_state` (target `routine`, action `create`) with the concrete name, prompt, and schedule or trigger. Do not stop at “set it up in its chat.” Confirm to the user that you messaged it to create the routine. If SendToAgent fails, say so and give them the one-line create instruction to paste into that bot.

## Output

Live bot whose `profile.json` matches the intended one-job / anti-jobs / voice / wake; user told name + one-job line; standing routine messaged for create when needed.

## Never

- Ship a shareable template unless the user asked for one
- Trust CreateAgent ack without reading live `profile.json`
- Pad description with leftover tools, model essays, or “I can also help with…”
- Paste the full pstack playbook into the description
- Re-create healthcheck routines that already exist
- Stop at “set it up in its chat” when a standing routine is required — SendToAgent the create instruction
